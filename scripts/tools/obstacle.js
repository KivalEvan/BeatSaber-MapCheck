        ...
        if (obstacle._type === 0) {
            if (obstacle._width > 2 || (obstacle._width > 1 && obstacle._lineIndex === 1)) {
                ...
                if (obstacle._lineIndex === 0) {
                    if (obstacleLonger(obstacle, obstacleLFull)) {
                        if (
                            isAboveTime(obstacle._time, toRealTime(obstacleRFull._time) - tool.obstacle.recovery) &&
                            isBelowTime(
                                obstacle._time,
                    ...
                    if (obstacleLonger(obstacle, obstacleRFull)) {
                        if (
                            isAboveTime(obstacle._time, toRealTime(obstacleLFull._time) - tool.obstacle.recovery) &&
                            isBelowTime(
                                obstacle._time,
...
function detectShortObstacle(diff, mapSettings) {
    ...
    for (let i = 0, len = obstacles.length; i < len; i++) {
        ...
        if (obstacle._type === 0 && obstacle._duration > 0) {
            if (obstacle._width > 2 || (obstacle._width > 1 && obstacle._lineIndex === 1)) {
                if (obstacleLonger(obstacle, obstacleLFull)) {
                    if (isBelowTime(obstacle._duration, tool.obstacle.minDur)) {
                        arr.push(adjustTime(obstacle._time, bpm, offset, bpmc));
                    }
                    obstacleLFull = obstacle;
                }
            ...
            if (obstacle._width > 2 || (obstacle._width > 1 && obstacle._lineIndex === 1)) {
                if (obstacleLonger(obstacle, obstacleLHalf)) {
                    if (
                        isBelowTime(obstacle._duration, tool.obstacle.minDur) &&
...
function obstacleLonger(w1, w2) {
    return isAboveTime(w1._time + w1._duration, toRealTime(w2._time + w2._duration));
}
// electric boogaloo; for <15ms obstacle
function obstacleLonger2(w1, w2) {
    return isAboveTime(w1._time + w1._duration, toRealTime(w2._time + w2._duration) + tool.obstacle.minDur);
}
