...
export class ColorNote extends WrapColorNote<IColorNoteContainer> {
   ...
   constructor(data: Partial<IWrapColorNoteAttribute<IColorNoteContainer>> = {}) {
      super();
      ...
      this._customData = deepCopy(data.customData ?? ColorNote.default.data.customData);
   }

   static fromJSON(object: Partial<IObjectLane> = {}, data: Partial<IColorNote> = {}): ColorNote {
      const d = new this();
      ...
      d._customData = deepCopy(data.customData ?? ColorNote.default.data.customData);
      return d;
   }
