...
function run(map: ToolArgs) {
    ...
    if (duration) {
        ...
        if (colorNotes.length && colorNotes[0].time < 0) {
            htmlString.push(
                `<b>Note(s) before start time:</b> ${round(colorNotes[0].time, 3)} (${toMMSS(
        ...
        if (obstacles.length && obstacles[0].time < 0) {
            htmlString.push(
                `<b>Obstacle(s) before start time:</b> ${round(obstacles[0].time, 3)} (${toMMSS(
                    bpm.toRealTime(obstacles[0].time),
                )})`,
            );
        }
        if (basicBeatmapEvents.length && basicBeatmapEvents[0].time < 0) {
            htmlString.push(
                `<b>Event(s) before start time:</b> ${round(basicBeatmapEvents[0].time, 3)} (${toMMSS(
                    bpm.toRealTime(basicBeatmapEvents[0].time),
                )})`,
            );
        }
        if (colorNotes.length && colorNotes[colorNotes.length - 1].time > endBeat) {
            htmlString.push(
                `<b>Note(s) after end time:</b> ${round(colorNotes[colorNotes.length - 1].time, 3)} (${toMMSS(
        ...
        if (obstacles.length && obstacles[obstacles.length - 1].time > endBeat) {
            htmlString.push(
                `<b>Obstacle(s) after end time:</b> ${round(obstacles[obstacles.length - 1].time, 3)} (${toMMSS(
        ...
        if (basicBeatmapEvents.length && basicBeatmapEvents[basicBeatmapEvents.length - 1].time > endBeat) {
            htmlString.push(
                `<b>Event(s) after end time:</b> ${round(
                    ...
                    3,
                )} (${toMMSS(bpm.toRealTime(basicBeatmapEvents[basicBeatmapEvents.length - 1].time))})`,
            );
        }
    }
